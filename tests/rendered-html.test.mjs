...
test("server-renders the Hypercourse shell and metadata", async () => {
  ...
  assert.match(html, /Learn HyperFrames locally through 47 hands-on lessons/i);
...
test("keeps Hypercourse naming consistent and migrates existing progress", async () => {
  ...
  assert.match(rootReadme, /47 hands-on lessons/);
});
...
test("includes a complete design.md and frame.md learning section", async () => {
  ...
  assert.equal(allLessons.length, 47);
  assert.match(
...
test("keeps responsive typography bounded and reflow-safe", async () => {
  ...
  assert.match(styles, /\.landing-title\s*\{[^}]*font-size:clamp\(4rem,[^}]*7\.25rem/s);
  assert.match(styles, /\.landing-copy\s*\{[^}]*grid-template-columns:minmax\(0, 1\.35fr\)/s);
